...
import sqlite3, json, datetime, os, os
...
class OpenChatGPTSettingsIn(BaseModel):
    api_key: str
    model: str = "gpt-4o-mini"
    image_model: str = "gpt-image-1"

class ChatGPTChatIn(BaseModel):
    message: str
    context_type: str = "general"

class ChatGPTTitleIn(BaseModel):
    product_name: str
    category: str = ""
    keywords: str = ""

class ChatGPTListingIn(BaseModel):
    product_name: str
    category: str = ""
    selling_points: str = ""
    price_rub: float = 0
...
class ChatGPTProductCopyIn(BaseModel):
    product_name: str
    category: str = ""
    material: str = ""
    size: str = ""
...
class ChatGPTProductImageIn(BaseModel):
    product_name: str
    category: str = ""
    selling_points: str = ""
...
class ChatGPTReplyIn(BaseModel):
    message_id: int
    tone: str = "礼貌专业"
...
def init_db():
    ...
    try:
        cur.execute("ALTER TABLE ai_settings ADD COLUMN image_model TEXT DEFAULT 'gpt-image-1'")
    except Exception:
        pass
    cur.execute("""
...
def get_ai_settings():
    ...
    if not row or not row["api_key"]:
        raise HTTPException(status_code=400, detail="请先在ChatGPT设置中填写 OpenChatGPT API Key")
    return dict(row)
...
def call_openai(messages, temperature=0.4):
    ...
    try:
        ...
        raise HTTPException(status_code=502, detail=f"OpenChatGPT API连接失败: {e}")
    if resp.status_code >= 400:
        raise HTTPException(status_code=resp.status_code, detail=resp.text)
    data = resp.json()
...
def save_ai_settings(s: OpenChatGPTSettingsIn, token: Optional[str] = None):
    require_role(token, ("admin",))
    c = conn()
    c.execute("""
    ...
    """, (s.api_key.strip(), s.model.strip() or "gpt-4o-mini", s.image_model.strip() or "gpt-image-1"))
    c.commit()
    c.close()
    return {"ok": True}
...
def read_ai_settings(token: Optional[str] = None):
    ...
    c.close()
    if not row:
        return {"model": "gpt-4o-mini", "image_model": "gpt-image-1", "api_key_set": False}
    return {"model": row["model"] or "gpt-4o-mini", "image_model": row["image_model"] or "gpt-image-1", "api_key_set": bool(row["api_key"])}

@app.post("/api/ai/chat")
def ai_chat(req: ChatGPTChatIn, token: Optional[str] = None):
    require_role(token, ("admin","manager","staff"))
...
def ai_ru_title(req: ChatGPTTitleIn, token: Optional[str] = None):
    require_role(token, ("admin","manager","staff"))
...
关键词：{req.keywords}
"""
    answer = call_openai([{"role": "user", "content": prompt}], temperature=0.3)
    save_ai_log("ru_title", prompt, answer)
    return {"ru_title": answer.strip()}

@app.post("/api/ai/listing")
def ai_listing(req: ChatGPTListingIn, token: Optional[str] = None):
    require_role(token, ("admin","manager","staff"))
...
售价卢布：{req.price_rub}
"""
    answer = call_openai([{"role": "user", "content": prompt}], temperature=0.35)
...
def try_translate_to_zh(text: str) -> str:
    # If ChatGPT API is configured, translate. Otherwise return original.
    try:
        answer = call_openai([
...
"""
    try:
        return call_openai([{"role": "user", "content": prompt}], temperature=0.35).strip()
    except Exception:
...
def ai_customer_reply(req: ChatGPTReplyIn, token: Optional[str] = None):
    require_role(token, ("admin","manager","staff"))
    c = conn()
...
def call_openai_image(prompt: str, size="1024x1024", quality="medium"):
    settings = get_ai_settings()
    # OpenChatGPT Image API returns base64-encoded image data.
...
def product_image_prompt(req: ChatGPTProductImageIn):
    type_map = {
...
def ai_product_copy(req: ChatGPTProductCopyIn, token: Optional[str] = None):
    require_role(token, ("admin","manager","staff"))
...
def ai_product_image(req: ChatGPTProductImageIn, token: Optional[str] = None):
    require_role(token, ("admin","manager","staff"))
    prompt = product_image_prompt(req)
    b64 = call_openai_image(prompt, req.size, req.quality)
    filename = f"product_{datetime.datetime.now().strftime('%Y%m%d%H%M%S%f')}.png"
