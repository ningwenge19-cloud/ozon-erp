...
class DoubaoSettingsIn(BaseModel):
    ...
    model: str = "ep-20260518230413-wm6nv"
...
def get_doubao_settings():
    ...
    env_model = os.environ.get("DOUBAO_MODEL") or "ep-20260518230413-wm6nv"
    ...
    try:
        ...
        CREATE TABLE IF NOT EXISTS doubao_settings (
            ...
            model TEXT DEFAULT 'ep-20260518230413-wm6nv',
            image_model TEXT DEFAULT 'doubao-seedream-3-0-t2i-250415'
        )
        """)
        try:
...
def doubao_settings_save(s: DoubaoSettingsIn, token: Optional[str] = None):
    ...
    """, (s.api_key.strip(), s.base_url.strip() or "https://ark.cn-beijing.volces.com/api/v3", s.model.strip() or "ep-20260518230413-wm6nv", s.image_model.strip() or "doubao-seedream-3-0-t2i-250415"))
    c.commit()
    c.close()
    return {"ok": True}
